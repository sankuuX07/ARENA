...
const TOPICS = [
  ...
  'Sentence Completion', 'Sentence Correction', 'Error Detection',
  'Grammar', 'Fill in the Blanks', 'Para Jumbles',
  'Idioms & Phrases', 'One Word Substitution', 'Analogies', 'Word Usage'
];

export const VerbalPage: React.FC = () => {
  ...
  const loadHistoryData = async (uid: string) => {
    try {
      ...
      setHistory(hist.filter(h => h.category === 'verbal'));
    } catch (e) {
  ...
  const handleStartSession = async () => {
    ...
    try {
      const data = await startAptitudeSession(currentUser.uid, 'verbal', difficulty, numQuestions, selectedTopic);
      setSession(data);
      setQuestions(data.questions);
    } catch (err: any) {
      setErrorMessage(err.message || 'Failed to start Verbal Ability session.');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div style={{ maxWidth: 1000, margin: '0 auto', width: '100%' }}>
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: '2rem' }}>
          ...
          <h2 className="section-title" style={{ margin: 0 }}>Verbal Ability History</h2>
        </div>
        
        {history.length === 0 ? (
          <EmptyState icon={<BarChart />} title="No History Yet" description="Complete your first verbal test to see your progress." />
        ) : (
          <div style={{ display: 'grid', gap: '1rem' }}>
            {history.map((h, i) => (
              <Card key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div style={{ fontWeight: 600, fontSize: '1.1rem' }}>{h.topic || 'Verbal'}</div>
    ...
    return (
      <div style={{ maxWidth: 800, margin: '0 auto', width: '100%' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <div>
            <h2 style={{ fontSize: '1.25rem', margin: 0 }}>{session.topic || 'Verbal Ability'}</h2>
    ...
    return (
      <div style={{ maxWidth: 800, margin: '0 auto', width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          ...
          <h2 style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>Verbal Ability Complete 🎉</h2>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2.5rem' }}>
        <div>
          ...
          <PageHeader
            ...
            description="Master verbal reasoning and language skills for elite placement tests."
