...
import { startCppSession, completeCppSession } from '../../../services/cppService';
...
export const CppSessionPage: React.FC = () => {
  ...
  const initSession = async () => {
    try {
      ...
      const newSession = await startCppSession(topic, difficulty, qType, count);
  ...
  const handleNext = async () => {
    ...
    if (currentIndex + 1 < session.questionCount) {
      ...
      try {
        setLoading(true);
        await completeCppSession(session.sessionId, finalScore);
        ...
        navigate('/technical/cpp/result', { state: { result: { score: finalScore, total: session.questionCount, topic: session.topic } }});
      } catch (err: any) {
    ...
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <h2 style={{ margin: 0, fontSize: '1.2rem', color: 'var(--text-secondary)' }}>
          Practice: <span style={{ color: 'var(--text-main)', textTransform: 'capitalize' }}>C++ / {session.topic.replace('_', ' ')}</span>
        </h2>
