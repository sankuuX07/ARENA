...
import { getPythonTopic } from '../../../services/pythonService';
...
export const PythonPracticePage: React.FC = () => {
  ...
  useEffect(() => {
    if (topicId) {
      getPythonTopic(topicId).then(setTopic).catch(console.error);
    }
  }, [topicId]);
  ...
  const handleStart = async () => {
    if (qType === 'coding') {
      ...
      navigate(`/technical/python/session/new?topic=${topicId}&difficulty=${difficulty}&qType=${qType}&count=${count}`);
    }
  };
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '2rem' }}>
      <Button variant="ghost" icon={<ArrowLeft size={16} />} onClick={() => navigate(`/technical/python`)} style={{ marginBottom: '1rem' }}>
