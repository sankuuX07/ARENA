...
export const CSResultPage: React.FC = () => {
  ...
  if (!result) {
    return (
      <div style={{ padding: '4rem', textAlign: 'center' }}>
        ...
        <br/>
        <Button onClick={() => navigate('/technical/cs-core')} style={{ marginTop: '1rem' }}>Back to Dashboard</Button>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto', padding: '2rem' }}>
      <Button variant="ghost" icon={<ArrowLeft size={16} />} onClick={() => navigate('/technical/cs-core')} style={{ marginBottom: '2rem' }}>
        Back to CS Core
      </Button>
      ...
      <div style={{ background: 'var(--bg-surface)', padding: '3rem', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-color)', textAlign: 'center' }}>
        ...
        <Button variant="primary" size="lg" fullWidth onClick={() => navigate('/technical/cs-core')}>
          Continue Learning
        </Button>
      </div>
    </div>
