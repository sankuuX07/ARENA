...
import { getCSCoreSubject, CSSubject, CSTopic } from '../../../services/csCoreService';
import { TechnicalDifficulty, TechnicalQuestionType } from '../../../services/technicalService';
import { generatePuzzleProblem } from '../../../services/puzzleService';

export const CSPracticePage: React.FC = () => {
  ...
  useEffect(() => {
    if (subjectId && topicId) {
      getCSCoreSubject(subjectId).then(sub => {
        setSubject(sub);
        const t = sub.topics?.find(x => x.topicId === topicId);
        if (t) setTopic(t);
      }).catch(console.error);
    }
  }, [subjectId, topicId]);
    ...
    if (qType === 'coding') {
      ...
      try {
        const p = await generatePuzzleProblem({
          category: `CS Core: ${subject?.name}`,
          difficulty: difficulty,
          problemType: 'coding',
          language: 'python', // Default to generic evaluation via python
          topic: topic?.name
        });
      ...
      navigate(`/technical/cs-core/session/new?subjectId=${subjectId}&topicId=${topicId}&difficulty=${difficulty}&qType=${qType}&count=${count}`);
    }
  };

  if (!subject || !topic) return <div style={{ padding: '2rem', textAlign: 'center' }}>Loading Topic...</div>;

  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '2rem' }}>
      <Button variant="ghost" icon={<ArrowLeft size={16} />} onClick={() => navigate(`/technical/cs-core/${subjectId}`)} style={{ marginBottom: '1rem' }}>
        ...
        <div style={{ display: 'grid', gap: '2rem', marginBottom: '2.5rem' }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
              {(['mcq', 'scenario', 'conceptual', 'coding', 'interview'] as TechnicalQuestionType[]).map((t) => (
