...
import { getJavaTopics } from '../../../services/javaService';
...
export const JavaProgrammingPage: React.FC = () => {
  ...
  if (loading) return <div style={{ padding: '2rem', textAlign: 'center' }}>Loading Java Programming Module...</div>;
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '2rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '2rem', gap: '2rem', flexWrap: 'wrap' }}>
        <div>
          <h1 style={{ fontSize: '2.5rem', marginBottom: '0.5rem' }}>Java Programming</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: '1.1rem' }}>
            Master Java and prepare for technical placement rounds.
          </p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1rem' }}>
        {topics.map(topic => (
          ...
          onClick={() => navigate(`/technical/java/${topic.topicId}`)}>
