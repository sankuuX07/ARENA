...
import { getCTopic } from '../../../services/cService';
...
export const CPracticePage: React.FC = () => {
  ...
  useEffect(() => {
    if (topicId) {
      getCTopic(topicId).then(setTopic).catch(console.error);
    }
  }, [topicId]);
  ...
  const handleStart = async () => {
    if (qType === 'coding') {
      ...
      navigate(`/technical/c/session/new?topic=${topicId}&difficulty=${difficulty}&qType=${qType}&count=${count}`);
    }
  };
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '2rem' }}>
      <Button variant="ghost" icon={<ArrowLeft size={16} />} onClick={() => navigate(`/technical/c`)} style={{ marginBottom: '1rem' }}>
        Back to C Topics
      </Button>
