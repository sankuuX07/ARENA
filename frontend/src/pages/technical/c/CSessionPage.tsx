...
import { startCSession, completeCSession } from '../../../services/cService';
...
export const CSessionPage: React.FC = () => {
  ...
  const initSession = async () => {
    try {
      ...
      const newSession = await startCSession(topic, difficulty, qType, count);
  ...
  const handleNext = async () => {
    ...
    if (currentIndex + 1 < session.questionCount) {
      ...
      try {
        setLoading(true);
        await completeCSession(session.sessionId, finalScore);
        ...
        navigate('/technical/c/result', { state: { result: { score: finalScore, total: session.questionCount, topic: session.topic } }});
      } catch (err: any) {
