...
import {
  ...
  getFormalSessionHistory,
  FormalEvaluationData,
  FormalHistoryItem,
  FormalStartResponseData,
} from '../services/formalCommunicationService';
...
const CATEGORY_OPTIONS = [
  { id: 'professional_introduction', label: 'Professional Introduction' },
  { id: 'interview_communication', label: 'Interview Communication' },
  { id: 'professional_email', label: 'Professional Email' },
  { id: 'workplace_conversation', label: 'Workplace Conversation' },
  { id: 'hr_communication', label: 'HR Communication' },
  { id: 'professional_request', label: 'Professional Request' },
  { id: 'professional_explanation', label: 'Professional Explanation' },
  { id: 'meeting_communication', label: 'Meeting Communication' },
  { id: 'presentation_communication', label: 'Presentation Communication' },
  { id: 'placement_communication', label: 'Placement Communication' },
];

export const FormalCommunicationPage: React.FC = () => {
  const navigate = useNavigate();
  const { currentUser } = useAuth();

  // Session State
  const [session, setSession] = useState<FormalStartResponseData | null>(null);
  ...
  const [turnEvaluations, setTurnEvaluations] = useState<FormalEvaluationData[]>([]);
  const [latestEvaluation, setLatestEvaluation] = useState<FormalEvaluationData | null>(null);
  ...
  const [history, setHistory] = useState<FormalHistoryItem[]>([]);
  ...
  const loadHistoryData = async (uid: string) => {
    try {
      const hist = await getFormalSessionHistory(uid);
      setHistory(hist);
    } catch (e) {
      console.error('[FormalCommunicationPage] Error loading history:', e);
    }
  };
  ...
  const handleStartSession = async () => {
    ...
    try {
      const data = await startFormalSession(currentUser.uid, selectedCategory, difficulty);
      setSession(data);
      setCurrentPrompt(data.scenario);
    } catch (err: any) {
      setErrorMessage(err.message || 'Failed to start Formal session.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmitTurn = async () => {
    ...
    if (responseText.trim().length < 5) {
      setErrorMessage('Please provide a meaningful response.');
      return;
    }
    ...
    try {
      const data = await submitFormalTurn(
        currentUser.uid,
        session.session_id,
        responseText.trim(),
        currentTurn,
      ...
      if (data.is_completed || currentTurn >= 5) {
        const completion = await completeFormalSession(
          currentUser.uid,
          session.session_id,
          session.category,
          session.difficulty,
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', minHeight: '50vh', alignItems: 'center', justifyContent: 'center' }}>
        <LoadingSpinner message="Starting Formal Communication Session..." size={22} />
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: 1050, margin: '0 auto', width: '100%' }}>
      <PageHeader
        title="Formal Communication Module"
        description="Master professional emails, corporate scenarios, HR interviews, and workplace etiquette."
          ...
          <Card style={{ gridColumn: 'span 2' }}>
            ...
            {history.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>
                Complete your first Formal Communication session to see your progress.
              </div>
            ) : (
      ...
      {session && !isSessionComplete && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h2 style={{ fontSize: '1.25rem', fontWeight: 600 }}>Formal Communication Practice</h2>
            <Badge variant="neutral">Turn {currentTurn} of 5</Badge>
          </div>
          ...
          {latestEvaluation && (
            <Card style={{ marginTop: '1rem', borderLeft: '4px solid var(--success-main)' }}>
              <h3 style={{ fontSize: '1.1rem', marginBottom: '1rem' }}>AI Feedback</h3>
              ...
              {latestEvaluation.betterVersion && (
                <div style={{ background: 'var(--bg-surface-elevated)', padding: '1rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-color)' }}>
                  <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-main)', marginBottom: '0.5rem' }}>PROFESSIONAL REWRITE</div>
                  <div style={{ fontSize: '0.95rem', color: 'var(--text-secondary)', marginBottom: '0.5rem', fontStyle: 'italic' }}>
                    "{latestEvaluation.betterVersion}"
                  </div>
                  {latestEvaluation.rewriteReason && (
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                      <strong>Why?</strong> {latestEvaluation.rewriteReason}
                    </div>
                  )}
                </div>
              )}
      ...
      {isSessionComplete && finalResult && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', alignItems: 'center' }}>
          <Card style={{ width: '100%', maxWidth: 700, textAlign: 'center', padding: '3rem 2rem' }}>
            ...
            <h2 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>Formal Communication Complete</h2>
