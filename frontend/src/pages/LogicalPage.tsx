...
const TOPICS = [
  'Number Series', 'Alphabet Series', 'Alphanumeric Series', 'Coding-Decoding',
  'Blood Relations', 'Direction Sense', 'Syllogisms', 'Analogies',
  'Classification', 'Odd One Out', 'Seating Arrangement', 'Ranking and Order',
  'Data Sufficiency', 'Statement and Conclusion', 'Statement and Assumption',
  'Cause and Effect', 'Assertion and Reason', 'Logical Venn Diagrams',
  'Calendar and Clock Reasoning', 'Puzzle-Based Reasoning'
];

export const LogicalPage: React.FC = () => {
  const navigate = useNavigate();
  const { currentUser } = useAuth();

  // Session State
  const [session, setSession] = useState<AptitudeStartResponse | null>(null);
  const [selectedTopic, setSelectedTopic] = useState<string>('Number Series');
  ...
  const loadHistoryData = async (uid: string) => {
    try {
      ...
      setHistory(hist.filter(h => h.category === 'logical'));
    } catch (e) {
  ...
  const handleStartSession = async () => {
    ...
    try {
      const data = await startAptitudeSession(currentUser.uid, 'logical', difficulty, numQuestions, selectedTopic);
      setSession(data);
      setQuestions(data.questions);
    } catch (err: any) {
      setErrorMessage(err.message || 'Failed to start Logical Reasoning session.');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div style={{ maxWidth: 1000, margin: '0 auto', width: '100%' }}>
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: '2rem' }}>
          ...
          <h2 className="section-title" style={{ margin: 0 }}>Logical Reasoning History</h2>
        </div>
        
        {history.length === 0 ? (
          <EmptyState icon={<BarChart />} title="No History Yet" description="Complete your first logical reasoning test to see your progress." />
        ) : (
          <div style={{ display: 'grid', gap: '1rem' }}>
            {history.map((h, i) => (
              <Card key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div style={{ fontWeight: 600, fontSize: '1.1rem' }}>{h.topic || 'Logical'}</div>
    ...
    return (
      <div style={{ maxWidth: 800, margin: '0 auto', width: '100%' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <div>
            <h2 style={{ fontSize: '1.25rem', margin: 0 }}>{session.topic || 'Logical Reasoning'}</h2>
        ...
        <Card style={{ marginBottom: '1.5rem', padding: '2rem' }}>
          <div style={{ fontSize: '1.1rem', lineHeight: 1.6, marginBottom: '2rem', whiteSpace: 'pre-line' }}>
            {currentQ.question}
          </div>
          ...
          {isReviewMode && currentQ.explanation && (
            <div style={{ marginTop: '2rem', padding: '1.5rem', background: 'var(--bg-surface)', borderRadius: 'var(--radius-md)', borderLeft: '4px solid var(--primary)' }}>
              <div style={{ fontWeight: 600, marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <Lightbulb size={18} /> Step-by-Step Reasoning
              </div>
    ...
    return (
      <div style={{ maxWidth: 800, margin: '0 auto', width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          ...
          <h2 style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>Logical Reasoning Complete 🎉</h2>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem', flexWrap: 'wrap' }}>
          <Button variant="outline" icon={<BrainCircuit size={18} />} onClick={() => { setIsReviewMode(true); setCurrentIndex(0); }}>
            Review Answers
          </Button>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2.5rem' }}>
        <div>
          ...
          <PageHeader
            ...
            description="Master logical and analytical reasoning for elite placement tests."
