...
class PythonSessionStartRequest(BaseModel):
    topic: str
...
async def get_python_topics():
    return python_service.get_topics()

@router.get("/topics/{topic_id}", response_model=TechnicalTopic)
async def get_python_topic(topic_id: str):
    topic = python_service.get_topic(topic_id)
    if not topic:
...
async def start_python_session(
    ...
    try:
        return await python_service.start_session(
            uid=uid,
            topic=request.topic,
...
async def get_python_session(
    ...
    session = python_service.get_session(uid, session_id)
    if not session:
